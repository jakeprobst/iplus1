...
int valid_word(iplus1_finnish_t* fin, char* s)
{
    if (strlen(s) == 0) {
        return 0;
    }
    if (fin->stopwords) {
        if (iplus1_tree_get(fin->stopwords, s)) {
            return 0;
        }
    }
    return 1;
...
void load_stopwords(iplus1_finnish_t* fin)
{
    fin->stopwords = NULL;
    FILE* stopfile = fopen(PREFIX "/share/iplus1/data/fin-stopwords.txt", "r");
    if (stopfile) {
        fin->stopwords = malloc(sizeof(iplus1_tree_t));
        iplus1_tree_init(fin->stopwords, &iplus1_tree_compare_str);
        ...
        while((ch = fgetc(stopfile)) != EOF) {
            if (ch == '\n') {
                word[w] = '\0';
                char* s = strdup(word);
                iplus1_tree_insert(fin->stopwords, s, s);
                w = 0;
            }
            else {
                word[w] = ch;
...
int destroy(iplus1_lang_t* lang)
{
    ...
    iplus1_tree_foreach_postorder(fin->stopwords, &iplus1_tree_free_key, NULL);
    iplus1_tree_destroy(fin->stopwords);
    free(fin->stopwords);
